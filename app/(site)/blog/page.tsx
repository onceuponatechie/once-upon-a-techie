...
/* Featured card — matches homepage */
    ...
    <Link href={`/blog/${post.slug}`} className="block h-full">
      ...
      >
        ...
        <div className="absolute top-5 left-5 z-10 flex items-center gap-3">
          <span className="text-[10px] font-sans font-semibold uppercase tracking-widest px-2.5 py-1 rounded-full" style={{ backgroundColor: "rgba(255,255,255,0.15)", color: "rgba(255,255,255,0.9)" }}>
            {post.category}
          </span>
          <span className="text-xs font-sans text-white/60">{post.date}</span>
          <span className="text-xs font-sans text-white/60">{post.readingTime}</span>
        </div>
    ...
    <Link href={`/blog/${post.slug}`} className="block h-full">
      ...
      >
        ...
        <div className="p-5 flex flex-col flex-1">
          <span className="text-[10px] font-sans font-semibold uppercase tracking-widest mb-2" style={{ color: catColor }}>
            {post.category}
          </span>
      ...
      <div className="max-w-6xl mx-auto">
        <ScrollReveal>
          ...
          <p className="font-sans text-text-secondary text-base max-w-xl mb-14">
            Thoughts on design, building products, creativity, and the messy journey of figuring it all out.
          </p>
        </ScrollReveal>

        {/* Featured + 2 stacked — matching homepage */}
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 mb-8" style={{ minHeight: "480px" }}>
